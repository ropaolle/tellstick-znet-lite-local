    ...
    return (
      <div className="dialog">
        <Modal isOpen={show} toggle={close} className={this.props.className}>
          ...
          <ModalBody>
            <div>
              <div>Expires: {date.toLocaleString()}, Allow renew: {Boolean(allowRenew).toString()}</div>
              <h4 className="mt-4">{step > 0 && <span className="text-success">&#x2714;</span>} Step 1. Request/renew token</h4>
              {step === 0 && <div>
                <Button onClick={this.handleRequestToken} color="warning">Request new token</Button>{' '}
                <Button onClick={this.handleRefreshToken} color="warning">Refresh token</Button>
              </div>}

              <h4 className="mt-4">{step > 1 && <span className="text-success">&#x2714;</span>} Step 2. Authenticate</h4>
              {step === 1 && <div>Go to<Button color="link" onClick={this.handleAuthentication}>{authUrl}</Button>and authenticate the app.</div>}

              <h4 className="mt-4">{step > 2 && <span className="text-success">&#x2714;</span>} Step 3. Access token</h4>
              <div>
